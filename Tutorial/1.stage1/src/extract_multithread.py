...
import time
import threading
from typing import Dict, Any, List, Iterable, Optional, Set, Tuple
...
- material: 材质/面料/成分（如：棉、纯棉、聚酯、涤纶、羊毛、真皮、PU、羽绒、锦纶、尼龙、亚麻、莱赛尔、粘纤等；可以包含配比）
...
- 纠错优先级：以原文附近上下文为证据；无证据则用行业常识做最小改动。
- 颜色纠错（重点）：
  - 颜色必须自然语言可理解；出现乱码/无意义组合必须修正或剔除。
  - 若能从同条记录中找到更合理颜色，用更合理的替换；
    若只能推断到大类，用大类（咖色/棕色/卡其/米色等）；
    若完全无依据，输出 []。
- 材质纠错：
  - 允许保留配比，但避免重复大类；优先保留更具体条目。
  - 明显不是材质的计量信息（如“30.5g充绒量”）丢弃，只保留材质本体。

C. 规范化与清洗
- 去重：同一字段内去重（保留首次出现顺序）。
- 清洗：去掉首尾空格、去掉明显无意义的包裹符号（如多余的【】），但保留内容本身。
...
def keep_model_output(raw: Dict[str, Any], fallback_skuid: str) -> Dict[str, Any]:
    """
    只做 skuid 兜底，其他字段原样保留（允许 list/None/任意）
    """
...
# =========================
# JSONL output helpers (thread-safe)
# =========================
_write_lock = threading.Lock()


def append_jsonl_threadsafe(path: str, obj: Dict[str, Any]) -> None:
...
# =========================
# Worker: process one batch (for multithreading)
# =========================
def process_batch(api_key: str, base_url: str, items: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], List[str]]:
    """
    返回：
    - outputs: 要写入 jsonl 的对象列表（每个对象一行）
    - skuids_done: 本批次实际完成的 skuid 列表（用于主线程更新 done_ids）
    """
    client = OpenAI(api_key=api_key, base_url=base_url)

    prompt = build_prompt_batch(items)
    ...
    for item in items:
        ...
        skuids_done.append(sid)

    return outputs, skuids_done
...
def main() -> None:
    ...
    # 预先组装 todo 的批次（避免多线程下 iter_jsonl 混乱）
    ...
    for batch in batched(iter_jsonl(INPUT_PATH), BATCH_SIZE):
        ...
        if todo_batch:
            batches.append(todo_batch)

    # tqdm 总进度依然按输入总量推进更符合原来体验：
    # - 已完成的会瞬间跳过（我们这里用 pbar.update 跳过数量）
    # 但现在我们预过滤了，所以改成：总量=待处理数量，更直观
    total_todo = sum(len(b) for b in batches)

    with tqdm(total=total_todo, desc=f"Extracting (threads={MAX_WORKERS})", unit="item") as pbar:
        futures = []
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as ex:
            for b in batches:
                futures.append(ex.submit(process_batch, api_key, base_url, b))

            for fut in as_completed(futures):
                try:
                    outputs, skuids_done = fut.result()
                except Exception as e:
                    # 不让单个批次把全局打崩：记录一下并继续
                    # 你也可以选择 raise
                ...
                # 写文件（线程安全）
                for obj in outputs:
                    append_jsonl_threadsafe(OUTPUT_JSONL_PATH, obj)

                # 更新 done_ids（主线程更新即可）
                for sid in skuids_done:
                    done_ids.add(sid)

                pbar.update(len(skuids_done))
